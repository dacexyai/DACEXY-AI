...
class Payment(Base):
    ...
    amount = Column(Integer, nullable=False)  # in paise (e.g., 199900 for ₹1999.00)
...
class UsageLog(Base):
    ...
    cost_rupees = Column(Integer, default=0)  # cost in paise (100 = ₹1)
    task_id = Column(String, nullable=True, index=True)
    status = Column(String, default="completed")  # reserved | completed | canceled
    created_at = Column(DateTime, default=lambda: datetime.now(timezone.utc), index=True)
