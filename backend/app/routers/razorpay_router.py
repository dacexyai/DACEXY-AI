...
def create_order(request: CreateOrderRequest, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    # Plan pricing (in paise) — final Beta pricing.
    # Free: ₹0 · Business: ₹1,999 · Enterprise: ₹9,999
    plans = {
        "business": {"amount": 199900, "description": "DACEXY Business - ₹1,999/month"},
        "enterprise": {"amount": 999900, "description": "DACEXY Enterprise - ₹9,999/month"},
    }
...
def create_subscription(request: CreateSubscriptionRequest, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    try:
        ...
        client = razorpay.Client(auth=(settings.razorpay_key_id, settings.razorpay_key_secret))
        expected_amounts = {"business": 199900, "enterprise": 999900}
        remote_plan = client.plan.fetch(plan_id)
        remote_amount = int((remote_plan.get("item") or {}).get("amount") or 0)
        if remote_amount != expected_amounts[request.plan] or (remote_plan.get("item") or {}).get("currency") != "INR":
            raise HTTPException(
                status_code=503,
                detail=f"Razorpay {request.plan} plan amount is not ₹{expected_amounts[request.plan] / 100:.0f}. Create/configure the final DACEXY plan ID before enabling subscriptions."
            )
