...
if (-not (Test-Path $electronExe)) {
  ...
  $zip = Get-ChildItem $cacheRoot -Recurse -Filter "electron-v$version-win32-x64.zip" -File -ErrorAction SilentlyContinue | Select-Object -First 1
...
if (-not (Test-Path $electronExe)) { throw "Electron executable is unavailable. Install Electron dependencies first." }
