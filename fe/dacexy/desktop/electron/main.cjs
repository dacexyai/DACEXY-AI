...
function installerRoot() {
  ...
  return candidates.find((p) => fs.existsSync(path.join(p, "install.ps1"))) || null;
}

function emitInstallerProgress(state) {
  const payload = state && typeof state === "object" ? state : { state: "running", message: String(state || "") };
  for (const win of BrowserWindow.getAllWindows()) {
    try { win.webContents.send("dacexy:installer-progress", payload); } catch (_) {}
  }
}
...
function agentInstalled() {
  ...
  const health = path.join(process.env.LOCALAPPDATA || "", "DACEXY", "runtime-health.json");
  if (!fs.existsSync(health)) return false;
  try {
    const h = JSON.parse(fs.readFileSync(health, "utf8"));
    return h?.product === "DACEXY AI" && h?.gateway === "ws://127.0.0.1:18789";
  } catch (_) {
    return false;
  }
}
...
function installAgentRuntime() {
  if (process.platform !== "win32") return Promise.reject(new Error("The DACEXY Agent installer currently supports Windows x64 only."));
  const root = installerRoot();
  if (!root) return Promise.reject(new Error("DACEXY Agent installer is missing from this application package."));
  if (installerProcess) return Promise.reject(new Error("DACEXY Agent installation is already running."));
  ...
  installerProcess = spawn(
    ...
    ],
    {
      windowsHide: true,
      cwd: root,
      env: { ...process.env, DACEXY_INSTALLER_UI: "1" },
  ...
  return new Promise((resolve, reject) => {
    ...
    const fail = (err) => {
      ...
      emitInstallerProgress({ state: "error", error: err instanceof Error ? err.message : String(err) });
    ...
    const finish = (code) => {
      ...
      if (code === 0) {
        settled = true;
        emitInstallerProgress({ state: "completed", step: 7, message: "DACEXY Agent installed. OpenClaw Gateway is running 24/7." });
        resolve({ installed: true, gateway: "ws://127.0.0.1:18789" });
      } else {
        fail(new Error(stderr.trim() || `DACEXY Agent installer exited with code ${code}.`));
      }
    };
    ...
    const consume = (chunk) => {
      ...
      for (const line of lines) {
        ...
        if (!text) continue;
        const m = text.match(/^DACEXY_PROGRESS\|(\d+)\|(\d+)\|(.*)$/);
        if (m) {
          emitInstallerProgress({
            state: "running",
            step: Number(m[1]),
            total: Number(m[2]),
            message: m[3],
          ...
          emitInstallerProgress({ state: "running", message: text.slice(0, 240) });
        }
      }
    };

    installerProcess.stdout.on("data", consume);
    installerProcess.stderr.on("data", (chunk) => {
      stderr += String(chunk || "");
      if (stderr.length > 2 * 1024 * 1024) stderr = stderr.slice(-2 * 1024 * 1024);
      consume(chunk);
    });
    installerProcess.on("error", fail);
    installerProcess.on("close", finish);
  });
}
...
function findOpenClawEntry() {
  const candidates = [
    ...
    path.join(process.env.LOCALAPPDATA || "", "DACEXY", "npm-global", "node_modules", "openclaw", "openclaw.mjs"),
  ].filter(Boolean);
