...
type OpenClawApi = { connect:(url:string)=>Promise<any>; disconnect:()=>Promise<any>; status:()=>Promise<{connected:boolean,error?:string}>; send:(text:string,sessionKey?:string)=>Promise<string>; onStatus:(cb:(s:{connected:boolean,error?:string})=>void)=>()=>void };
...
export class GatewayClient {
  ...
  private api(){ const api=(window as DWin).dacexy?.openclaw; if(!api) throw new Error("DACEXY agent bridge is unavailable"); return api; }
  ...
  async sendMessage(text:string){ await this.connect(); const r=await this.api().send(text); if(r)this.onReply?.(r); return r||"The agent returned no response."; }
