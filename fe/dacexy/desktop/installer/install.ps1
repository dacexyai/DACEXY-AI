...
$total = 7
function Progress($n,$msg){ Write-Output ("DACEXY_PROGRESS|{0}|{1}|{2}" -f $n,$total,$msg) }
...
Progress 1 "Checking Node.js runtime"
...
Progress 2 "Installing OpenClaw agent engine"
...
Progress 3 "Configuring low-credit AI runtime"
...
# Cost/latency guardrails: keep every interactive agent turn compact and fast.
# These do not remove tools or capabilities; they only disable expensive deliberation and cap a turn.
try { Run "openclaw" @("config", "set", "agents.defaults.thinkingDefault", "off") } catch { Write-Host "  Thinking-default optimization unavailable; continuing with provider defaults." -ForegroundColor Yellow }
try { Run "openclaw" @("config", "set", "agents.defaults.timeoutSeconds", "300") } catch { Write-Host "  Timeout optimization unavailable; continuing with provider defaults." -ForegroundColor Yellow }
try { Run "openclaw" @("config", "set", "agents.defaults.fastModeDefault", "true") } catch { Write-Host "  Fast-mode optimization unavailable; continuing with provider defaults." -ForegroundColor Yellow }
if ($providerKey) {
    try { Run "openclaw" @("config", "set", "models.providers.deepseek.maxTokens", "800") } catch { Write-Host "  DeepSeek output-token cap could not be applied; continuing safely." -ForegroundColor Yellow }
}

# 4. ALWAYS apply DACEXY branding/configuration for this installer run (display-only).
Step 4 $total "Applying DACEXY branding..."
Progress 4 "Applying DACEXY runtime configuration"
...
Progress 5 "Installing business skills"
...
Progress 6 "Enabling full OpenClaw capabilities"
...
Progress 7 "Starting 24/7 OpenClaw Gateway"
$providerKey = $env:DACEXY_DEEPSEEK_API_KEY
...
# Keep the Gateway loopback-only, but require a generated shared secret even on localhost.
# The desktop client/CLI reads the credential locally; no secret is exposed in the UI.
try { Run "openclaw" @("doctor", "--generate-gateway-token") } catch { Write-Host "  Gateway token generation returned a non-fatal status; existing auth will be reused." -ForegroundColor Yellow }
