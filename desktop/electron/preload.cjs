...
contextBridge.exposeInMainWorld("dacexy", {
  ...
  agent: {
    status: () => ipcRenderer.invoke("dacexy:agent-status"),
    install: () => ipcRenderer.invoke("dacexy:agent-install"),
    onProgress: (cb) => { const l = (_e, state) => cb(state); ipcRenderer.on("dacexy:installer-progress", l); return () => ipcRenderer.removeListener("dacexy:installer-progress", l); },
  },
