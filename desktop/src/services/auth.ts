...
export async function signUp(first: string, last: string, email: string, company: string, password: string): Promise<Account> {
  let res: Response;
  try {
    res = await fetch(`${API_URL}/api/v1/auth/signup`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ first: first.trim(), last: last.trim(), email: email.trim(), company: company.trim() || null, password }),
    });
  } catch { throw new Error("Unable to connect to the DACEXY AI server."); }
  const data = await res.json().catch(() => null);
  if (!res.ok) throw new Error(errorMessage(data, `Sign up failed (HTTP ${res.status})`));
  const { access_token, user } = data;
  const licRes = await fetch(`${API_URL}/api/v1/license/verify`, {
    method: "POST",
    headers: { "Content-Type": "application/json", Authorization: `Bearer ${access_token}` },
    body: JSON.stringify({ key: "", machine_id: getMachineId() }),
  });
  const licData = await licRes.json().catch(() => null);
  if (!licRes.ok || !licData?.valid) throw new Error(errorMessage(licData, "License verification failed"));
  const account: Account = { name: `${user.first_name} ${user.last_name}`, email: user.email, plan: user.plan, token: access_token };
  localStorage.setItem(KEY, JSON.stringify(account));
  return account;
}
